...
public class ReviewBoardNotifierTest {
    ...
    public void setupGlobalConfig() {
        ...
        ReviewBoardServerConfiguration serverConfig =
                new ReviewBoardServerConfiguration(REVIEWBOARD_URL, REVIEWBOARD_API_TOKEN);
    ...
    public void addBuildParameters(final FreeStyleProject project) throws IOException {
        ...
        final StringParameterDefinition reviewId =
                new StringParameterDefinition("REVIEWBOARD_REVIEW_ID", String.valueOf(REVIEW_ID));
        final StringParameterDefinition statusUpdateId =
                new StringParameterDefinition("REVIEWBOARD_STATUS_UPDATE_ID", String.valueOf(STATUS_UPDATE_ID));

        project.addProperty(new ParametersDefinitionProperty(reviewId, statusUpdateId, serverURL));
    }
    ...
    public void testConfigRoundtrip() throws Exception {
        ...
        jenkins.assertEqualDataBoundBeans(
                new ReviewBoardNotifier(), project.getPublishersList().get(0));
    }
    ...
    public void testBuildWithStatuses() throws Exception {
        testBuildStatus(
                ...
                org.reviewboard.rbjenkins.Messages.ReviewBoard_Job_Success());

        testBuildStatus(
                Result.ABORTED,
                ReviewRequest.StatusUpdateState.ERROR_STATE,
                org.reviewboard.rbjenkins.Messages.ReviewBoard_Job_Aborted());

        testBuildStatus(
                Result.NOT_BUILT,
                ReviewRequest.StatusUpdateState.ERROR_STATE,
                org.reviewboard.rbjenkins.Messages.ReviewBoard_Job_NotBuilt());

        testBuildStatus(
                Result.UNSTABLE,
                ReviewRequest.StatusUpdateState.FAILURE_STATE,
                org.reviewboard.rbjenkins.Messages.ReviewBoard_Job_Unstable());

        testBuildStatus(
                Result.FAILURE,
                ReviewRequest.StatusUpdateState.FAILURE_STATE,
                org.reviewboard.rbjenkins.Messages.ReviewBoard_Job_Failure());
    }

    private void testBuildStatus(Result result, ReviewRequest.StatusUpdateState status, String message)
            throws Exception {
        setupGlobalConfig();
        final FreeStyleProject project = jenkins.createFreeStyleProject();
    ...
    public void testBuildUpdateError() throws Exception {
        ...
        ReviewBoardNotifier publisher = new ReviewBoardNotifier();
        publisher = Mockito.spy(publisher);
        Mockito.doThrow(new ReviewBoardException(""))
                .when(publisher)
                .updateStatusUpdate(
                        ArgumentMatchers.any(ReviewRequest.class),
                        ArgumentMatchers.eq(ReviewRequest.StatusUpdateState.SUCCESS_STATE),
                        ArgumentMatchers.eq(org.reviewboard.rbjenkins.Messages.ReviewBoard_Job_Success()));
        project.getPublishersList().add(publisher);
        ...
        jenkins.assertLogContains("Unable to notify Review Board of the result of the build:", build);
    }
}
